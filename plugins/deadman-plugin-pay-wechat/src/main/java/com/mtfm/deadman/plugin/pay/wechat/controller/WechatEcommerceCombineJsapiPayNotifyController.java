...
import com.mtfm.deadman.plugin.pay.wechat.provider.WechatEcommerceCombineJsapiPaymentProvider;
...
/**
 * 微信收付通合单 JSAPI 支付结果回调 Controller。
 * <p>
...
@ConditionalOnProperty(
        ...
        name = "enabled",
        havingValue = "true")
public class WechatEcommerceCombineJsapiPayNotifyController {

    private final PayService payService;
    private final WechatPayPluginProperties wechatPayPluginProperties;

    /**
     * 接收微信收付通合单 JSAPI 支付结果回调。
     *
     * @param body    回调请求体原文
     ...
     */
    @PostMapping(
            "${deadman.plugin.pay-wechat.providers.wechat-ecommerce-combine-jsapi.notify-endpoint:"
    ...
    public WechatPayNotifyAck notify(@RequestBody String body, HttpServletRequest request) {
        ...
        try {
            payService.handleNotify(
                    WechatEcommerceCombineJsapiPaymentProvider.PROVIDER_ID,
                    WechatPayNotifyHttpUtils.toNotifyContext(body, request));
            return WechatPayNotifyAck.success();
        } catch (Exception ex) {
            log.warn(
                    "微信收付通合单 JSAPI 支付回调处理失败：provider={}, endpoint={}",
                    WechatEcommerceCombineJsapiPaymentProvider.PROVIDER_ID,
