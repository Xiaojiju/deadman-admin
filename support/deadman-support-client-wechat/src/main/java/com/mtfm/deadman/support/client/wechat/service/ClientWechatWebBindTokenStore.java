...
public class ClientWechatWebBindTokenStore {
     ...
     * @return 临时绑定令牌
     */
    public String store(ClientWechatWebPendingSession session) {
        String bindToken = UUID.randomUUID().toString().replace("-", "");
        String key = RedisKeyConstants.clientWechatWebBindKey(bindToken);
     ...
     * @return 待绑定会话
     */
    public ClientWechatWebPendingSession consume(String bindToken) {
        if (!StringUtils.hasText(bindToken)) {
            throw new BusinessException(ResultCode.WECHAT_BIND_TOKEN_INVALID);
        }
        String key = RedisKeyConstants.clientWechatWebBindKey(bindToken.trim());
        String payload = stringRedisTemplate.opsForValue().getAndDelete(key);
        if (!StringUtils.hasText(payload)) {
            throw new BusinessException(ResultCode.WECHAT_BIND_TOKEN_INVALID);
        }
        try {
            return jsonMapper.readValue(payload, ClientWechatWebPendingSession.class);
        } catch (Exception ex) {
            throw new BusinessException(ResultCode.WECHAT_BIND_TOKEN_INVALID);
        }
    }
