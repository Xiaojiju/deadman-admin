...
/**
 * 用户端微信网页扫码 OAuth 绑定注册 Provider：携带 bindToken 注册新用户并完成 openid 绑定，成功后签发 JWT。
 */
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(prefix = "deadman.plugin.wechat-web", name = "enabled", havingValue = "true")
public class ClientWechatWebRegisterLoginProvider implements LoginProvider {

    /** 绑定注册路径 */
    public static final String REGISTER_LOGIN_ENDPOINT = "/client/api/auth/wechat-web/register";
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        return clientWechatWebAuthService.registerAndBind(
                ...
                request.avatarFileId(), request.phone(), request.inviteCode());
    }
