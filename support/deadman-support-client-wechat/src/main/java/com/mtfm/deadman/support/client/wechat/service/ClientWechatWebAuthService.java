...
public class ClientWechatWebAuthService {
    ...
    private final ClientWechatWebBindTokenStore bindTokenStore;

    /**
     * 使用网页授权 code 完成用户端微信登录：已绑定则直接认证，未绑定则返回待绑定令牌。
     *
     * @param code  微信授权临时凭证
    ...
    public Authentication loginByWechatWebCode(String code, String state) {
        ...
        String bindToken = bindTokenStore.store(new ClientWechatWebPendingSession(
                webSession.openid(),
                webSession.unionid(),
                webSession.nickname(),
    ...
    public Authentication bindAndAuthenticate(String bindToken, String username, String password) {
        ClientWechatWebPendingSession session = consumeBindSession(bindToken);
     ...
     * @param password  密码
     * @param nickname  昵称，可为空（为空时回退微信会话昵称）
     * @param avatar    头像 URL，可为空（为空时回退微信会话头像）
     * @return 已认证的用户端登录用户
     */
    @Transactional(rollbackFor = Exception.class)
    public Authentication registerAndBind(String bindToken, String username, String password, String nickname,
            String avatar) {
        ClientWechatWebPendingSession session = consumeBindSession(bindToken);
        String resolvedNickname = StringUtils.hasText(nickname) ? nickname.trim() : session.nickname();
        String resolvedAvatar = StringUtils.hasText(avatar) ? avatar.trim() : session.headimgurl();
        ClientUserBase userBase;
        try {
            userBase = clientAuthCredentialsService.registerUser(
                    new ClientRegisterRequest(username.trim(), password, resolvedNickname, resolvedAvatar));
        ...
        clientUserAccountService.bindOAuth(
                userBase.getId(), WechatWebConstants.OAUTH_PROVIDER, session.openid());
    ...
    private ClientWechatWebPendingSession consumeBindSession(String bindToken) {
        try {
